...
import com.sleeve.tianqinrag.utils.PasswordUtil;
    ...
    public OrganizationTag updateOrganizationTag(String tagId, String name, String description,
        ...
        if (parentTag != null && !parentTag.isEmpty()) {
            ...
            if (wouldFormCycle(tagId, parentTag)) {
                throw new CustomException("Setting this parent would create a cycle in the tag hierarchy",
        ...
        for (User user : users) {
            if (user.getOrgTags() != null && !user.getOrgTags().isEmpty()) {
                ...
                if (tagId.equals(user.getPrimaryOrg())) {
                    throw new CustomException("Cannot delete a tag that is used as primary organization",
                            HttpStatus.CONFLICT);
                }
            }
        }
